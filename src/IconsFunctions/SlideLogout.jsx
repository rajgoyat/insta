...
navigate('/insta/edit')
  ...
  const handleSaved=()=>{
    setborderTop(3)
    setshowwhat("saved")
    navigate(`/insta/profile/${user.userId}`)
  }
    return(<div className='slidelogoutIconmain'> <div onClick={()=>setallshow("")} className=" slidelogoutIcon position-absolute bg-white d-flex flex-column  justify-content-evenly" style={{height:"404px", width:"266px", bottom:"50px",left:"55px", boxShadow:"0 0 10px 2px rgba(43, 41, 41, 0.274)",borderRadius:"20px"}}>
      ...
      <div onClick={handleSaved} className='d-flex set' style={{margin:"4px",fontSize:"14px",color:"black", padding:"6px"}}><div><Saved/></div><div className='ms-2'>Saved</div></div>
