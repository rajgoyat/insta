...
export default function FollowingPage() {
    ...
    const { user, following, setFollowing,followUser,unfollowUser } = firebase; // Get logged-in user
   ...
   const handleFollow = async (event, userToFollow) => {
    ...
    return (
        <>
            <div className={`${following ? "" : "d-none"} w-100 bg- DALJU position-absolute top-0`} onClick={() => setFollowing(false)} style={{ height: "100vh", background: "rgba(0,0,0,0.65)", zIndex: "1001" }}>
      ...
      e.stopPropagation()}}>
                    <div style={{ height: "42px" }} className="w-100 d-flex align-items-center justify-content-between px-2">
                        ...
                        <div className='cr' onClick={() => setFollowing(false)}><Cross /></div>
                    </div>
                        ...
                        {filterData.map((val, id) => {
                          return ( <div key={val.id} className='w-100 d-flex bg- align-items-center justify-content-between' style={{ height: "60px", padding: "8px 16px" }}>
                            <div className='d-flex gap-2 align-items-center'>
                                <div style={{ width: "54px", height: "54px", borderRadius: "50%" }} className='DALJU bg- cr' onClick={()=>navigate(`/SearchProfile/${val.id}`)}>
                            ...
                            <div className='DALJU cr' style={{ width: "95px", height: "32px", borderRadius: "8px", padding: "0px 10px", fontSize: "14px", fontWeight: "700", background: "#efefef" }}>Following
  ...
  {loading ? (
                            ...
                            followingUsers.map(user => (
                                <div key={user.id} className='w-100 d-flex bg- align-items-center justify-content-between' style={{ height: "60px", padding: "8px 16px" }}>
                                    <div className='d-flex gap-2 align-items-center'>
                                        <div style={{ width: "54px", height: "54px", borderRadius: "50%" }} className='DALJU bg- cr' onClick={()=>navigate(`/SearchProfile/${user.id}`)}>
                                    ...
                                    {/* <div className='DALJU cr' style={{ width: "95px", height: "32px", borderRadius: "8px", padding: "0px 10px", fontSize: "14px", fontWeight: "700", background: "#efefef" }}>jaat</div> */}
