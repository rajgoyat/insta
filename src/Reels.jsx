...
{reels.map((val,ind)=>{
  return(
    <div key={ind} className="position-relative m-2 d-flex">
      <div className="soundBtnReels position-absolute fs-3 rounded-circle d-flex align-items-center justify-content-center " onClick={()=>setVol(!vol)} style={{zIndex:"2", background:"rgba(0, 0, 0, 0.763)",  height:"35px", width:"35px"}}>{vol ?<IoVolumeHigh size={26} color="white"/> :<IoVolumeMute size={26} color="white"/> }</div>

          <div className="position-absolute reelData text-white" style={{ zIndex: "2" }}>
          <div className=" d-flex flex-row"><img className="rounded-circle" src={val.img} alt="" style={{height:"32px", width:"32px" }}/><div className="ms-2 me-1 container" style={{fontSize:"15px"}}>{val.channelName}</div><div className="container text-white" style={{height:"33px",width:"100px",border:"1px solid white",borderRadius:"8px"}}>Follow</div></div>
          <div style={{fontSize:"15px"}}>{val.videoName} ...more</div>
            <div className="blurdiv mt-2 mb-2"><div><Song/> {val.songName}</div></div>
          </div>
          {/* <video
...
const VideoComponent = ({ src, vol ,setVol }) => {
  ...
  return (
    <div ref={videoContainerRef} className="video-container position-relative">
      ...
      <video ref={videoRef} src={src} controls={false} {...(vol ? {} : {muted: true})}  className="position-relative" style={{zIndex:'1', borderRadius: "10px"}}/>
    </div>
  );
};
